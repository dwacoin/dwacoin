...
public final class DebugTrace {
    ...
    static final String QUOTE = Dwa.getStringProperty("dwa.debugTraceQuote", "\"");
    static final String SEPARATOR = Dwa.getStringProperty("dwa.debugTraceSeparator", "\t");
    static final boolean LOG_UNCONFIRMED = Dwa.getBooleanProperty("dwa.debugLogUnconfirmed");

    static void init() {
        List<String> accountIdStrings = Dwa.getStringListProperty("dwa.debugTraceAccounts");
        String logName = Dwa.getStringProperty("dwa.debugTraceLog");
        ...
        Dwa.getBlockchainProcessor().addListener(block -> debugTrace.resetLog(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public static DebugTrace addDebugTrace(Set<Long> accountIds, String logName) {
        ...
        Dwa.getBlockchainProcessor().addListener(debugTrace::traceBeforeAccept, BlockchainProcessor.Event.BEFORE_BLOCK_ACCEPT);
        Dwa.getBlockchainProcessor().addListener(debugTrace::trace, BlockchainProcessor.Event.BEFORE_BLOCK_APPLY);
        Dwa.getTransactionProcessor().addListener(transactions -> debugTrace.traceRelease(transactions.get(0)), TransactionProcessor.Event.RELEASE_PHASED_TRANSACTION);
    ...
    private void traceShufflingCancel(Shuffling shuffling) {
        ...
        if (blamedAccountId != 0 && include(blamedAccountId)) {
            ...
            long generatorId = Dwa.getBlockchain().getLastBlock().getGeneratorId();
            if (include(generatorId)) {
                Map<String,String> generatorMap = getValues(generatorId, false);
                generatorMap.put("generation fee", String.valueOf(fee));
                generatorMap.put("event", "shuffling blame");
    ...
    private Map<String,String> lessorGuaranteedBalance(Account account, long lesseeId) {
        ...
        map.put("timestamp", String.valueOf(Dwa.getBlockchain().getLastBlock().getTimestamp()));
        map.put("height", String.valueOf(Dwa.getBlockchain().getHeight()));
    ...
    private Map<String,String> getValues(long accountId, Shuffling shuffling, boolean isRecipient) {
        ...
        if (shuffling.getHoldingType() == HoldingType.DWA) {
    ...
    private Map<String,String> getValues(long accountId, Block block) {
        ...
        if (block.getHeight() > Constants.SHUFFLING_BLOCK) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                if (include(previousGeneratorId)) {
                    ...
                    map.put("effective balance", String.valueOf(Account.getAccount(previousGeneratorId).getEffectiveBalanceDWA()));
        ...
        Map<String,String> map = getValues(accountId, false);
        map.put("effective balance", String.valueOf(Account.getAccount(accountId).getEffectiveBalanceDWA()));
