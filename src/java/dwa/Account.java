...
import dwa.AccountLedger.LedgerEntry;
import dwa.AccountLedger.LedgerEvent;
import dwa.AccountLedger.LedgerHolding;
import dwa.crypto.Crypto;
import dwa.crypto.EncryptedData;
import dwa.db.DbClause;
import dwa.db.DbIterator;
import dwa.db.DbKey;
import dwa.db.DbUtils;
import dwa.db.DerivedDbTable;
import dwa.db.VersionedEntityDbTable;
import dwa.db.VersionedPersistentDbTable;
import dwa.util.Convert;
import dwa.util.Listener;
import dwa.util.Listeners;
...
public final class Account {
    ...
    public static final class AccountAsset {
        ...
        private void save(Connection con) throws SQLException {
            try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO account_asset "
                ...
                pstmt.setInt(++i, Dwa.getBlockchain().getHeight());
                pstmt.executeUpdate();
            }
        }
    ...
    public static final class PublicKey {
        ...
        private PublicKey(long accountId, byte[] publicKey) {
            ...
            this.height = Dwa.getBlockchain().getHeight();
        }
        ...
        private void save(Connection con) throws SQLException {
            height = Dwa.getBlockchain().getHeight();
    ...
    private static final VersionedEntityDbTable<AccountAsset> accountAssetTable = new VersionedEntityDbTable<AccountAsset>("account_asset", accountAssetDbKeyFactory) {
        ...
        public void checkAvailable(int height) {
            if (height + Constants.MAX_DIVIDEND_PAYMENT_ROLLBACK < Dwa.getBlockchainProcessor().getMinRollbackHeight()) {
                throw new IllegalArgumentException("Historical data as of height " + height +" not available.");
            }
            if (height > Dwa.getBlockchain().getHeight()) {
                throw new IllegalArgumentException("Height " + height + " exceeds blockchain height " + Dwa.getBlockchain().getHeight());
            }
        }
    ...
    private static final ConcurrentMap<DbKey, byte[]> publicKeyCache = Dwa.getBooleanProperty("dwa.enablePublicKeyCache") ?
    ...
    static {
        ...
        Dwa.getBlockchainProcessor().addListener(block -> {
        ...
        if (publicKeyCache != null) {
            ...
            Dwa.getBlockchainProcessor().addListener(block -> publicKeyCache.clear(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public long getEffectiveBalanceDWA() {
        return getEffectiveBalanceDWA(Dwa.getBlockchain().getHeight());
    }

    public long getEffectiveBalanceDWA(int height) {
        if (height >= Constants.TRANSPARENT_FORGING_BLOCK_6) {
            if (this.publicKey == null) {
                this.publicKey = publicKeyTable.get(accountDbKeyFactory.newKey(this));
            }
            if (this.publicKey == null || this.publicKey.publicKey == null || this.publicKey.height == 0 || height - this.publicKey.height <= Constants.MAX_ANNOUNCE) {
        ...
        if (height < Constants.TRANSPARENT_FORGING_BLOCK_3) {
            if (Arrays.binarySearch(Genesis.GENESIS_RECIPIENTS, id) >= 0) {
                return balanceNQT / Constants.ONE_DWA;
            }
            long receivedInLastBlock = 0;
            for (Transaction transaction : Dwa.getBlockchain().getBlockAtHeight(height).getTransactions()) {
                if (id == transaction.getRecipientId()) {
                    receivedInLastBlock += transaction.getAmountNQT();
                }
            }
            return (balanceNQT - receivedInLastBlock) / Constants.ONE_DWA;
        }
        Dwa.getBlockchain().readLock();
        try {
            ...
            return (height > Constants.SHUFFLING_BLOCK && effectiveBalanceNQT < Constants.MIN_FORGING_BALANCE_NQT) ? 0 : effectiveBalanceNQT / Constants.ONE_DWA;
        } finally {
            Dwa.getBlockchain().readUnlock();
        }
    }
    ...
    private long getLessorsGuaranteedBalanceNQT(int height) {
        ...
        int blockchainHeight = Dwa.getBlockchain().getHeight();
        try (Connection con = Db.db.getConnection();
    ...
    public long getGuaranteedBalanceNQT() {
        return getGuaranteedBalanceNQT(Constants.GUARANTEED_BALANCE_CONFIRMATIONS, Dwa.getBlockchain().getHeight());
    }
    ...
    public long getGuaranteedBalanceNQT(final int numberOfConfirmations, final int currentHeight) {
        ...
        try {
            ...
            if (height + Constants.GUARANTEED_BALANCE_CONFIRMATIONS < Dwa.getBlockchainProcessor().getMinRollbackHeight()
                    || height > Dwa.getBlockchain().getHeight()) {
    ...
    void leaseEffectiveBalance(long lesseeId, int period) {
        int height = Dwa.getBlockchain().getHeight();
    ...
    static boolean setOrVerify(long accountId, byte[] key) {
        ...
        if (publicKey.publicKey == null) {
            ...
            publicKey.height = Dwa.getBlockchain().getHeight();
            return true;
        }
    ...
    void apply(byte[] key) {
        ...
        } else if (publicKey.height >= Dwa.getBlockchain().getHeight() - 1) {
    ...
    void addToAssetBalanceQNT(LedgerEvent event, long eventId, long assetId, long quantityQNT) {
        addToAssetBalanceQNT(event, eventId, assetId, quantityQNT,0);
    }
    void addToAssetBalanceQNT(LedgerEvent event, long eventId, long assetId, long quantityQNT,long feeQNT) {
        if (quantityQNT == 0 && feeQNT == 0) {
            return;
        }
        ...
        long assetBalanceBeforeFee = Math.addExact(assetBalance, quantityQNT);
        assetBalance = Math.addExact(assetBalanceBeforeFee, feeQNT);
        ...
        if (AccountLedger.mustLogEntry(this.id, false)) {
            AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id, LedgerHolding.ASSET_BALANCE, assetId,
                    quantityQNT, assetBalanceBeforeFee));
            if(event==LedgerEvent.ASSET_TRANSFER||event==LedgerEvent.ASSET_ASK_ORDER_PLACEMENT||event==LedgerEvent.ASSET_ASK_ORDER_CANCELLATION){
                LedgerEvent eventFee = LedgerEvent.ASSET_TRANSFER_FEE;
                if(event==LedgerEvent.ASSET_ASK_ORDER_PLACEMENT){
                    eventFee = LedgerEvent.ASSET_ASK_ORDER_FEE;
                }else if(event == LedgerEvent.ASSET_ASK_ORDER_CANCELLATION){
                    eventFee = LedgerEvent.ASSET_ASK_CANCELLATION_FEE;
                }
                AccountLedger.logEntry(new LedgerEntry(eventFee, eventId, this.id, LedgerHolding.ASSET_BALANCE, assetId,
                        feeQNT, assetBalance));
            }
        }
    }
    ...
    void addToUnconfirmedAssetBalanceQNT(LedgerEvent event, long eventId, long assetId, long quantityQNT) {
        addToUnconfirmedAssetBalanceQNT(event, eventId, assetId, quantityQNT,0);
    }
    void addToUnconfirmedAssetBalanceQNT(LedgerEvent event, long eventId, long assetId, long quantityQNT,long feeQNT) {
        if (quantityQNT == 0 && feeQNT == 0) {
            return;
        }
        AccountAsset accountAsset = accountAssetTable.get(accountAssetDbKeyFactory.newKey(this.id, assetId)); //SENDER
        long unconfirmedAssetBalance = accountAsset == null ? 0 : accountAsset.unconfirmedQuantityQNT;
        Logger.logDebugMessage("addToUnconfirmedAssetBalanceQNT.unconfirmedAssetBalance:"+unconfirmedAssetBalance+",quantityQNT:"+quantityQNT);
        long unconfirmedAssetBalanceBeforeFee = Math.addExact(unconfirmedAssetBalance, quantityQNT);
        unconfirmedAssetBalance = Math.addExact(unconfirmedAssetBalanceBeforeFee, feeQNT);
        Logger.logDebugMessage("addToUnconfirmedAssetBalanceQNT.unconfirmedAssetBalance.afterAdd:"+unconfirmedAssetBalance);
        if (accountAsset == null) {
        ...
        if (AccountLedger.mustLogEntry(this.id, true)) {
            AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                    ...
                    quantityQNT, unconfirmedAssetBalanceBeforeFee));
            if(event==LedgerEvent.ASSET_TRANSFER||event==LedgerEvent.ASSET_ASK_ORDER_PLACEMENT||event==LedgerEvent.ASSET_ASK_ORDER_CANCELLATION){
                LedgerEvent eventFee = LedgerEvent.ASSET_TRANSFER_FEE;
                if(event==LedgerEvent.ASSET_ASK_ORDER_PLACEMENT){
                    eventFee = LedgerEvent.ASSET_ASK_ORDER_FEE;
                ...
                AccountLedger.logEntry(new LedgerEntry(eventFee, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_ASSET_BALANCE, assetId,
                        feeQNT, unconfirmedAssetBalance));
            }
        }
    }

    void addToAssetAndUnconfirmedAssetBalanceQNT(LedgerEvent event, long eventId, long assetId, long quantityQNT) {
        addToAssetAndUnconfirmedAssetBalanceQNT(event, eventId, assetId, quantityQNT,0);
    }
    void addToAssetAndUnconfirmedAssetBalanceQNT(LedgerEvent event, long eventId, long assetId, long quantityQNT,long feeQNT) {
        if (quantityQNT == 0 && feeQNT == 0) {
            return;
        }
        AccountAsset accountAsset;
        ...
        if (AccountLedger.mustLogEntry(this.id, false)) {
            if (feeNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.DWA_BALANCE, null, feeNQT, this.balanceNQT - amountNQT));
            }
            if (amountNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.DWA_BALANCE, null, amountNQT, this.balanceNQT));
            }
        }
    }
    ...
    void addToUnconfirmedBalanceNQT(LedgerEvent event, long eventId, long amountNQT, long feeNQT) {
        ...
        if (AccountLedger.mustLogEntry(this.id, true)) {
            if (feeNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_DWA_BALANCE, null, feeNQT, this.unconfirmedBalanceNQT - amountNQT));
            }
            if (amountNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_DWA_BALANCE, null, amountNQT, this.unconfirmedBalanceNQT));
            }
        }
    }
