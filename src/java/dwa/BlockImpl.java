...
final class BlockImpl implements Block {
    ...
    static BlockImpl parseBlock(JSONObject blockData) throws DwaException.NotValidException {
        try {
            ...
            if (!block.checkSignature()) {
                throw new DwaException.NotValidException("Invalid block signature");
            }
            return block;
    ...
    byte[] bytes() {
        if (bytes == null) {
            ...
            if (version < 3) {
                buffer.putInt((int) (totalAmountNQT / Constants.ONE_DWA));
                buffer.putInt((int) (totalFeeNQT / Constants.ONE_DWA));
            } else {
    ...
    boolean verifyGenerationSignature() throws BlockchainProcessor.BlockOutOfOrderException {
        ...
        try {
            ...
            long effectiveBalance = account == null ? 0 : account.getEffectiveBalanceDWA();
    ...
    void apply() {
        ...
        if (this.height > Constants.SHUFFLING_BLOCK) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                Logger.logDebugMessage("Back fees %f DWA to forger at height %d", ((double)backFees[i])/Constants.ONE_DWA, this.height - i - 1);
        ...
        if (totalBackFees != 0) {
            Logger.logDebugMessage("Fee reduced by %f DWA at height %d", ((double)totalBackFees)/Constants.ONE_DWA, this.height);
        }
    ...
    private void calculateBaseTarget(BlockImpl previousBlock) {
        ...
        if (previousBlock.getHeight() < Constants.SHUFFLING_BLOCK) {
            ...
            if (baseTarget < (prevBaseTarget * 3) / 5 ) {
                baseTarget = (prevBaseTarget * 3) / 5;
            }
            if (baseTarget == 0) {
                baseTarget = 1;
            }
