...
public final class ShufflingParticipant {
    ...
    void setData(byte[][] data, int timestamp) {
        if (data != null && Dwa.getEpochTime() - timestamp < Constants.MAX_PRUNABLE_LIFETIME && getData() == null) {
            shufflingDataTable.insert(new ShufflingData(shufflingId, accountId, data, timestamp, Dwa.getBlockchain().getHeight()));
        }
    }
