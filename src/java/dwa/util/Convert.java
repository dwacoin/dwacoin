...
public final class Convert {
    ...
    public static long parseAccountId(String account) {
        ...
        long accountId = 0L;
        if (prefixEnd > 0) {
            accountId = Crypto.rsDecode(account.substring(prefixEnd + 1));
        } else if (prefixEnd == 0) {
    ...
    public static String rsAccount(long accountId) {
        return "DWA-" + Crypto.rsEncode(accountId);
    }
    ...
    public static String readString(ByteBuffer buffer, int numBytes, int maxLength) throws DwaException.NotValidException {
        if (numBytes > 3 * maxLength) {
            throw new DwaException.NotValidException("Max parameter length exceeded");
        }
    ...
    public static long parseDWA(String dwa) {
        return parseStringFraction(dwa, 8, Constants.MAX_BALANCE_DWA);
    }
