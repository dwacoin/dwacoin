...
/**
 * Handle logging for the Dwa node server
 */
public final class Logger {
     ...
     * The dwa/conf/logging-default.properties and dwa/conf/logging.properties configuration
    ...
    static {
        ...
        System.setProperty("java.util.logging.manager", "dwa.util.DwaLogManager");
        if (!(LogManager.getLogManager() instanceof DwaLogManager)) {
            System.setProperty("java.util.logging.manager",
                    (oldManager != null ? oldManager : "java.util.logging.LogManager"));
        }
        if (! Boolean.getBoolean("dwa.doNotConfigureLogging")) {
            try {
                Properties loggingProperties = new Properties();
                Dwa.loadProperties(loggingProperties, "logging-default.properties", true);
                Dwa.loadProperties(loggingProperties, "logging.properties", false);
                Dwa.updateLogFileHandler(loggingProperties);
        ...
        log = org.slf4j.LoggerFactory.getLogger(dwa.Dwa.class);
        enableStackTraces = Dwa.getBooleanProperty("dwa.enableStackTraces");
        enableLogTraceback = Dwa.getBooleanProperty("dwa.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof DwaLogManager) {
            ((DwaLogManager) LogManager.getLogManager()).dwaShutdown();
        }
    }
