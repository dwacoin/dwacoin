...
public interface Fee {
    ...
    Fee DEFAULT_FEE = new Fee.ConstantFee(Constants.ONE_DWA);
    ...
    final class ConstantFee implements Fee {
        ...
        public long getFee(TransactionImpl transaction, Appendix appendage) {
            return (fee * Constants.FEE_NOMINATOR) / Constants.FEE_DENOMINATOR;
        }
    ...
    abstract class SizeBasedFee implements Fee {
        ...
        public final long getFee(TransactionImpl transaction, Appendix appendage) {
            ...
            if (size < 0) {
                return (this.constantFee * Constants.FEE_NOMINATOR) / Constants.FEE_DENOMINATOR;
            }

            return ( Math.addExact(this.constantFee, Math.multiplyExact((long) (size / unitSize), this.feePerSize) ) * Constants.FEE_NOMINATOR / Constants.FEE_DENOMINATOR );
        }
