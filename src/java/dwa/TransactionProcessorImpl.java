...
final class TransactionProcessorImpl implements TransactionProcessor {
    ...
    private static final boolean enableTransactionRebroadcasting = Dwa.getBooleanProperty("dwa.enableTransactionRebroadcasting");
    private static final boolean testUnconfirmedTransactions = Dwa.getBooleanProperty("dwa.testUnconfirmedTransactions");
    private static final int maxUnconfirmedTransactions;
    static {
        int n = Dwa.getIntProperty("dwa.maxUnconfirmedTransactions");
    ...
    private final Runnable removeUnconfirmedTransactionsThread = () -> {
        ...
        try {
            try {
                if (Dwa.getBlockchainProcessor().isDownloading() && ! testUnconfirmedTransactions) {
                    return;
                }
                ...
                try (DbIterator<UnconfirmedTransaction> iterator = unconfirmedTransactionTable.getManyBy(
                        new DbClause.IntClause("expiration", DbClause.Op.LT, Dwa.getEpochTime()), 0, -1, "")) {
    ...
    private final Runnable processTransactionsThread = () -> {
        ...
        try {
            try {
                ...
                JSONObject response = peer.send(JSON.prepareRequest(request), 20 * 1024 * 1024);
                if (response == null) {
                    return;
                }
                ...
                } catch (DwaException.ValidationException|RuntimeException e) {
                    peer.blacklist(e);
                }
            } catch (Exception e) {
    ...
    @Override
    public long countUnconfirmedTransaction() {
        return unconfirmedTransactionTable.getRowCount();
    }
    ...
    @Override
    public void broadcast(Transaction transaction) throws DwaException.ValidationException {
        BlockchainImpl.getInstance().writeLock(); //SUSPECT
        try {
    ...
    @Override
    public void processPeerTransactions(JSONObject request) throws DwaException.ValidationException {
    ...
    void processWaitingTransactions() {
        ...
        try {
            if (waitingTransactions.size() > 0) {
                int currentTime = Dwa.getEpochTime();
                ...
                while (iterator.hasNext()) {
                    ...
                    } catch (DwaException.ExistingTransactionException e) {
                        iterator.remove();
                    } catch (DwaException.NotCurrentlyValidException e) {
    ...
    private void processPeerTransactions(JSONArray transactionsData) throws DwaException.NotValidException {
        if (Dwa.getBlockchain().getHeight() <= Constants.LAST_KNOWN_BLOCK && !testUnconfirmedTransactions) {
            return;
        }
        ...
        for (Object transactionData : transactionsData) {
            ...
            } catch (DwaException.NotCurrentlyValidException ignore) {
            } catch (DwaException.ValidationException|RuntimeException e) {
        ...
        if (!exceptions.isEmpty()) {
            throw new DwaException.NotValidException("Peer sends invalid transactions: " + exceptions.toString());
        }
    }

    private void processTransaction(UnconfirmedTransaction unconfirmedTransaction) throws DwaException.ValidationException {
        Logger.logDebugMessage("processTransaction.unconfirmedTransaction "+unconfirmedTransaction.getTransaction().getId()+":"+unconfirmedTransaction.getAmountNQT()+":"+unconfirmedTransaction.getFeeNQT()+":"+unconfirmedTransaction.getAttachment().getTransactionType().getName());
        TransactionImpl transaction = unconfirmedTransaction.getTransaction();
        int curTime = Dwa.getEpochTime();
        if (transaction.getTimestamp() > curTime + Constants.MAX_TIMEDRIFT || transaction.getExpiration() < curTime) {
            Logger.logDebugMessage("pTuT:E1");
            throw new DwaException.NotCurrentlyValidException("Invalid transaction timestamp");
        }
        if (transaction.getVersion() < 1) {
            Logger.logDebugMessage("pTuT:E2");
            throw new DwaException.NotValidException("Invalid transaction version");
        }
        if (transaction.getId() == 0L) {
            Logger.logDebugMessage("pTuT:E3");
            throw new DwaException.NotValidException("Invalid transaction id 0");
        }
        ...
        try {
            try {
                Db.db.beginTransaction();
                if (Dwa.getBlockchain().getHeight() <= Constants.LAST_KNOWN_BLOCK && !testUnconfirmedTransactions) {
                    Logger.logDebugMessage("pTuT:E4");
                    throw new DwaException.NotCurrentlyValidException("Blockchain not ready to accept transactions");
                }

                if (getUnconfirmedTransaction(transaction.getDbKey()) != null || TransactionDb.hasTransaction(transaction.getId())) {
                    Logger.logDebugMessage("pTuT:E5");
                    throw new DwaException.ExistingTransactionException("Transaction already processed");
                }

                if (! transaction.verifySignature()) {
                    if (Account.getAccount(transaction.getSenderId()) != null) {
                        Logger.logDebugMessage("pTuT:E6");
                        throw new DwaException.NotValidException("Transaction signature verification failed");
                    } else {
                        throw new DwaException.NotCurrentlyValidException("Unknown transaction sender");
                    }
                }

                if (! transaction.applyUnconfirmed()) {
                    Logger.logDebugMessage("pTuT:E7");
                    throw new DwaException.InsufficientBalanceException("Insufficient balance");
                }

                if (transaction.isUnconfirmedDuplicate(unconfirmedDuplicates)) {
                    Logger.logDebugMessage("pTuT:E8");
                    throw new DwaException.NotCurrentlyValidException("Duplicate unconfirmed transaction");
                }

                unconfirmedTransactionTable.insert(unconfirmedTransaction);

                Db.db.commitTransaction();
                Logger.logDebugMessage("pTuT:C");
            } catch (Exception e) {
                Logger.logDebugMessage("pTuT:E9");
                Db.db.rollbackTransaction();
                throw e;
            } finally {
                Db.db.endTransaction();
     ...
     * @throws  DwaException.NotValidException    Transaction is not valid
     */
    @Override
    public List<Transaction> restorePrunableData(JSONArray transactions) throws DwaException.NotValidException {
