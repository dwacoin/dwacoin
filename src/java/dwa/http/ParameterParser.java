...
import dwa.Currency;
import dwa.CurrencyBuyOffer;
import dwa.CurrencySellOffer;
...
import static dwa.http.JSONResponses.*;
...
public final class ParameterParser {
    ...
    public static int getNumberOfConfirmations(HttpServletRequest req) throws ParameterException {
        return getInt(req, "numberOfConfirmations", 0, Dwa.getBlockchain().getHeight(), false);
    }
    ...
    public static int getHeight(HttpServletRequest req) throws ParameterException {
        ...
        if (heightValue != null) {
            try {
                ...
                if (height < 0 || height > Dwa.getBlockchain().getHeight()) {
    ...
    public static long getHoldingId(HttpServletRequest req, HoldingType holdingType) throws ParameterException {
        long holdingId = ParameterParser.getUnsignedLong(req, "holding", holdingType != HoldingType.DWA);
        if (holdingType == HoldingType.DWA && holdingId != 0) {
            throw new ParameterException(JSONResponses.incorrect("holding", "holding id should not be specified if holdingType is DWA"));
        }
        return holdingId;
    }
    ...
    public static Transaction.Builder parseTransaction(String transactionJSON, String transactionBytes, String prunableAttachmentJSON) throws ParameterException {
        ...
        if (transactionJSON != null) {
            try {
                ...
                return Dwa.newTransactionBuilder(json);
            } catch (DwaException.ValidationException | RuntimeException | ParseException e) {
            ...
            try {
                ...
                return Dwa.newTransactionBuilder(bytes, prunableAttachments);
            } catch (DwaException.ValidationException|RuntimeException | ParseException e) {
                Logger.logDebugMessage(e.getMessage(), e);
                JSONObject response = new JSONObject();
    ...
    public static Attachment.TaggedDataUpload getTaggedData(HttpServletRequest req) throws ParameterException, DwaException.NotValidException {
