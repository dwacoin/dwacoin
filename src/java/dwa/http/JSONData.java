...
import dwa.AccountRestrictions;
import dwa.Alias;
import dwa.Appendix;
import dwa.Asset;
import dwa.AssetDelete;
import dwa.AssetDividend;
import dwa.AssetTransfer;
import dwa.Attachment;
import dwa.Block;
import dwa.Constants;
import dwa.Currency;
import dwa.CurrencyExchangeOffer;
import dwa.CurrencyFounder;
import dwa.CurrencyTransfer;
import dwa.CurrencyType;
import dwa.DigitalGoodsStore;
import dwa.Exchange;
import dwa.ExchangeRequest;
import dwa.FundingMonitor;
import dwa.Generator;
import dwa.HoldingType;
import dwa.MonetarySystem;
import dwa.Dwa;
import dwa.Order;
import dwa.PhasingPoll;
import dwa.PhasingVote;
import dwa.Poll;
import dwa.PrunableMessage;
import dwa.Shuffler;
import dwa.Shuffling;
import dwa.ShufflingParticipant;
import dwa.TaggedData;
import dwa.Token;
import dwa.Trade;
import dwa.Transaction;
import dwa.Vote;
import dwa.VoteWeighting;
import dwa.crypto.Crypto;
import dwa.crypto.EncryptedData;
import dwa.db.DbIterator;
import dwa.peer.Hallmark;
...
public final class JSONData {
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance) {
        return accountBalance(account, includeEffectiveBalance, Dwa.getBlockchain().getHeight());
    }
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance, int height) {
        ...
        if (account == null) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceDWA", "0");
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceDWA", account.getEffectiveBalanceDWA(height));
    ...
    static JSONObject lessor(Account account, boolean includeEffectiveBalance) {
        ...
        if (accountLease.getCurrentLesseeId() != 0) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceDWA", String.valueOf(account.getGuaranteedBalanceNQT() / Constants.ONE_DWA));
            }
        }
    ...
    static JSONObject shuffling(Shuffling shuffling, boolean includeHoldingInfo) {
        ...
        if (includeHoldingInfo && shuffling.getHoldingType() != HoldingType.DWA) {
        ...
        if (includeExecutedPhased) {
            ...
            try (DbIterator<PhasingPoll.PhasingPollResult> phasingPollResults = PhasingPoll.getApproved(block.getHeight())) {
                for (PhasingPoll.PhasingPollResult phasingPollResult : phasingPollResults) {
                    ...
                    if (includeTransactions) {
                        phasedTransactions.add(transaction(Dwa.getBlockchain().getTransaction(phasedTransactionId)));
                    } else {
    ...
    static JSONObject pollResults(Poll poll, List<Poll.OptionResult> results, VoteWeighting voteWeighting) {
        ...
        if (voteWeighting.getMinBalanceModel() == VoteWeighting.MinBalanceModel.ASSET) {
            ...
            if (currency != null) {
                ...
                Transaction currencyIssuance = Dwa.getBlockchain().getTransaction(voteWeighting.getHoldingId());
    ...
    static JSONObject transaction(Transaction transaction, Filter<Appendix> filter) {
        ...
        json.put("confirmations", Dwa.getBlockchain().getHeight() - transaction.getHeight());
    ...
    private static void putAssetInfo(JSONObject json, long assetId) {
        ...
        json.put("name", asset==null?"":asset.getName());
        json.put("decimals", asset==null?0:asset.getDecimals());
    }

    private static void putExpectedTransaction(JSONObject json, Transaction transaction) {
        json.put("height", Dwa.getBlockchain().getHeight() + 1);
        json.put("phased", transaction.getPhasing() != null);
    ...
    static void ledgerEntry(JSONObject json, LedgerEntry entry, boolean includeTransactions, boolean includeHoldingInfo) {
        ...
        if (includeTransactions && entry.getEvent().isTransaction()) {
            Transaction transaction = Dwa.getBlockchain().getTransaction(entry.getEventId());
