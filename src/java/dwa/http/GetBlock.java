...
import static dwa.http.JSONResponses.INCORRECT_BLOCK;
import static dwa.http.JSONResponses.INCORRECT_HEIGHT;
import static dwa.http.JSONResponses.INCORRECT_TIMESTAMP;
import static dwa.http.JSONResponses.UNKNOWN_BLOCK;
...
public final class GetBlock extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        if (blockValue != null) {
            try {
                blockData = Dwa.getBlockchain().getBlock(Convert.parseUnsignedLong(blockValue));
            ...
            try {
                ...
                blockData = Dwa.getBlockchain().getBlockAtHeight(height);
            } catch (RuntimeException e) {
                return INCORRECT_HEIGHT;
            }
            ...
            try {
                ...
                blockData = Dwa.getBlockchain().getLastBlock(timestamp);
            } catch (RuntimeException e) {
                return INCORRECT_TIMESTAMP;
            }
        } else {
            blockData = Dwa.getBlockchain().getLastBlock();
        }
