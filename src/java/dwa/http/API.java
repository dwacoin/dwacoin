...
import static dwa.http.JSONResponses.MISSING_ADMIN_PASSWORD;
import static dwa.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static dwa.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
import static dwa.http.JSONResponses.NO_PASSWORD_IN_CONFIG;
...
public final class API {
    ...
    public static final String adminPassword = Dwa.getStringProperty("dwa.adminPassword", "", true);
    static final boolean disableAdminPassword;
    public static final int maxRecords = Dwa.getIntProperty("dwa.maxAPIRecords");
    static final boolean enableAPIUPnP = Dwa.getBooleanProperty("dwa.enableAPIUPnP");
    public static final int apiServerIdleTimeout = Dwa.getIntProperty("dwa.apiServerIdleTimeout");
    public static final boolean apiServerCORS = Dwa.getBooleanProperty("dwa.apiServerCORS");
    private static final String forwardedForHeader = Dwa.getStringProperty("dwa.forwardedForHeader");
    ...
    static {
        List<String> disabled = new ArrayList<>(Dwa.getStringListProperty("dwa.disabledAPIs"));
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = Dwa.getStringListProperty("dwa.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = Dwa.getStringListProperty("dwa.allowedBotHosts");
        ...
        boolean enableAPIServer = Dwa.getBooleanProperty("dwa.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : Dwa.getIntProperty("dwa.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : Dwa.getIntProperty("dwa.apiServerSSLPort");
            final String host = Dwa.getStringProperty("dwa.apiServerHost");
            disableAdminPassword = Dwa.getBooleanProperty("dwa.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = Dwa.getBooleanProperty("dwa.apiSSL");
            //
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(Dwa.getUserHomeDir()).resolve(Paths.get(Dwa.getStringProperty("dwa.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(Dwa.getStringProperty("dwa.keyStorePassword", null, true));
                ...
                sslContextFactory.setKeyStoreType(Dwa.getStringProperty("dwa.keyStoreType"));
                List<String> ciphers = Dwa.getStringListProperty("dwa.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = Dwa.getStringProperty("dwa.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{Dwa.getStringProperty("dwa.apiWelcomeFile")});
            }

            String javadocResourceBase = Dwa.getStringProperty("dwa.javadocResourceBase");
            ...
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/dwa");
            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(Dwa.getIntProperty("dwa.maxUploadFileSize"), Constants.MAX_TAGGED_DATA_DATA_LENGTH), -1L, 0));

            servletHolder = apiHandler.addServlet(APIProxyServlet.class, "/dwa-proxy");
            ...
            if (!Dwa.getBooleanProperty("dwa.enableAPIServerGZIPFilter", isOpenAPI)) {
                gzipHandler.setExcludedPaths("/dwa", "/dwa-proxy");
            }
            gzipHandler.setIncludedMethods("GET", "POST");
            gzipHandler.setMinGzipSize(dwa.peer.Peers.MIN_COMPRESS_SIZE);
            ...
            if (Dwa.getBooleanProperty("dwa.apiFrameOptionsSameOrigin")) {
