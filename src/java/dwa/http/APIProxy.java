...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (Dwa.getBooleanProperty("dwa.enableAPIProxy") && ! API.isOpenAPI);
    private static final int blacklistingPeriod = Dwa.getIntProperty("dwa.apiProxyBlacklistingPeriod") / 1000;
    static final String forcedServerURL = Dwa.getStringProperty("dwa.forceAPIProxyServerURL", "");
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && Dwa.getBlockchainProcessor().isDownloading());
    }
    ...
    boolean blacklistHost(String host) {
        ...
        blacklistedPeers.put(host, Dwa.getEpochTime() + blacklistingPeriod);
