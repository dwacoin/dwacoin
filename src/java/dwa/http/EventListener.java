...
import dwa.TransactionProcessor;
 ...
 * has not been received within dwa.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by dwa.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = Dwa.getIntProperty("dwa.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(Dwa.getIntProperty("dwa.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = Dwa.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = Dwa.getTransactionProcessor();
    ...
    /** Dwa event listeners */
    private final List<DwaEventListener> dwaEventListeners = new ArrayList<>();
     ...
     *
     * Dwa event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of Dwa event registrations
     ...
     *
     * @param   eventRegistrations      Dwa event registrations
     * @throws  EventListenerException  Invalid Dwa event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<DwaEventListener> it = dwaEventListeners.iterator();
                while (it.hasNext()) {
                    DwaEventListener listener = it.next();
                ...
                if (addListener) {
                    DwaEventListener listener = new DwaEventListener(event);
                    listener.addListener();
                    dwaEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            dwaEventListeners.forEach(DwaEventListener::removeListener);
        } finally {
            lock.unlock();
        }
     ...
     * Dwa event listener
     */
    private class DwaEventListener {

        /** Event handler */
        private final DwaEventHandler eventHandler;

        /**
         * Create the Dwa event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public DwaEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the Dwa listener for this event
         */
        ...
        /**
         * Remove the Dwa listener for this event
         */
        ...
        /**
         * Return the hash code for this Dwa event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two Dwa events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof DwaEventListener) &&
                    eventHandler.equals(((DwaEventListener)obj).eventHandler));
        }

        /**
         * Dwa listener event handler
         */
        private abstract class DwaEventHandler {
            ...
            /**
             * Create the Dwa event handler
             *
             * @param   eventRegistration   Event registration
             */
            public DwaEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof DwaEventHandler) &&
                                        owner == ((DwaEventHandler)obj).owner &&
                                        accountId == ((DwaEventHandler)obj).accountId &&
                                        event == ((DwaEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends DwaEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends DwaEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends DwaEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends DwaEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           Dwa listener event
        ...
        /**
         * Return the Dwa listener event
         *
         * @return                  Dwa listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
