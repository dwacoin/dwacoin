...
import static dwa.http.JSONResponses.FEATURE_NOT_AVAILABLE;
import static dwa.http.JSONResponses.INCORRECT_DEADLINE;
import static dwa.http.JSONResponses.INCORRECT_EC_BLOCK;
import static dwa.http.JSONResponses.INCORRECT_LINKED_FULL_HASH;
import static dwa.http.JSONResponses.INCORRECT_WHITELIST;
import static dwa.http.JSONResponses.MISSING_DEADLINE;
import static dwa.http.JSONResponses.MISSING_SECRET_PHRASE;
import static dwa.http.JSONResponses.NOT_ENOUGH_FUNDS;
...
abstract class CreateTransaction extends APIServlet.APIRequestHandler {
    ...
    private Appendix.Phasing parsePhasing(HttpServletRequest req) throws ParameterException {
        int finishHeight = ParameterParser.getInt(req, "phasingFinishHeight",
                Dwa.getBlockchain().getHeight() + 1,
                Dwa.getBlockchain().getHeight() + Constants.MAX_PHASING_DURATION + 1,
                true);
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
                                            long amountNQT, Attachment attachment) throws DwaException {
        return createTransaction(req, senderAccount,null, recipientId, amountNQT, attachment);
    }
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, Account issuerAccount, long recipientId,
                                            long amountNQT, Attachment attachment) throws DwaException {
        ...
        if (ecBlockId != 0 && ecBlockId != Dwa.getBlockchain().getBlockIdAtHeight(ecBlockHeight)) {
            return INCORRECT_EC_BLOCK;
        }
        if (ecBlockId == 0 && ecBlockHeight > 0) {
            ecBlockId = Dwa.getBlockchain().getBlockIdAtHeight(ecBlockHeight);
        }
        ...
        try {
            Transaction.Builder builder = Dwa.newTransactionBuilder(publicKey, amountNQT, feeNQT,
            ...
            try {
//                //ORIGINAL LOGIC, if amountNQT+feeNQT > sender.uncofirmedbalanceNQT
//                if (Math.addExact(amountNQT, transaction.getFeeNQT()) > senderAccount.getUnconfirmedBalanceNQT()) {
//                    return NOT_ENOUGH_FUNDS;
//                }
                //NEW LOGIC
                if(issuerAccount!=null){
                    //If amountNQT > sender.unconfirmedbalanceNQT --> Not enough OR
                    //If feeNQT > issuer.unconfirmedbalanceNQT --> Not enough
                    if (
                            amountNQT > senderAccount.getUnconfirmedBalanceNQT() ||
                                    feeNQT > issuerAccount.getUnconfirmedBalanceNQT()
                    ) {
                        return NOT_ENOUGH_FUNDS;
                    }
                }else{
            ...
            } catch (DwaException.NotYetEncryptedException ignore) {}
...
//                Logger.logInfoMessage("BroadcastTime:"+ (new Date().getTime()-now));//Suspect
                response.put("broadcasted", true);
            } else {
                transaction.validate();
                response.put("broadcasted", false);
            }
        } catch (DwaException.NotYetEnabledException e) {
