 ...
 *
 *   java -cp "classes:lib/*:conf" dwa.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Ddwa.runtime.mode=desktop dwa.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize Dwa properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "dwa.testDb" : "dwa.db";
        String dbType = Dwa.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Dwa.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Dwa.getDbDir(Dwa.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Dwa.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = Dwa.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Dwa.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "dwa.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "dwa.mv.db");
            if (!dbFile.exists()) {
