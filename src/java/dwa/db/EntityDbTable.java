...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public void checkAvailable(int height) {
        if (multiversion) {
            int minRollBackHeight = isPersistent() && Dwa.getBlockchainProcessor().isScanning() ?
                    Math.max(Dwa.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
                    : Dwa.getBlockchainProcessor().getMinRollbackHeight();
    ...
    public final DbIterator<T> getAllRaw(int from, int to) {
        Connection con = null;
        try {
            con = db.getConnection();
            PreparedStatement pstmt = con.prepareStatement("SELECT * FROM " + table
    ...
    public final void insertRaw(T t) {
    ...
    private boolean doesNotExceed(int height) {
        return Dwa.getBlockchain().getHeight() <= height && ! (isPersistent() && Dwa.getBlockchainProcessor().isScanning());
    }
