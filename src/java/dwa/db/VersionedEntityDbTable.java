...
public abstract class VersionedEntityDbTable<T> extends EntityDbTable<T> {
    ...
    public final boolean delete(T t, boolean keepInCache) {
        ...
        try (Connection con = db.getConnection();
            ...
            pstmtCount.setInt(i, Dwa.getBlockchain().getHeight());
    ...
    public final boolean deleteRaw(DbClause dbClause) {
        try (Connection con = db.getConnection();PreparedStatement pstmtDelete = con.prepareStatement("DELETE FROM " + table + " WHERE 1 AND "+dbClause.getClause())) {
            int i = 0;
            i = dbClause.set(pstmtDelete, ++i);
            return pstmtDelete.executeUpdate() > 0;
        } catch (SQLException e) {
            throw new RuntimeException(e.toString(), e);
        }
