...
import dwa.model.DWAReconnectionParameter;
import dwa.util.Convert;
import dwa.util.CountingInputReader;
...
final class PeerImpl implements Peer {
    ...
    Map<String, DWAReconnectionParameter> mapReconnection = Collections.synchronizedMap(new HashMap<String, DWAReconnectionParameter>());
    ...
    void setVersion(String version) {
        ...
        isOldVersion = false;
        if (Dwa.APPLICATION.equals(application)) {
...
//        if (state != null) {
//            Logger.logDebugMessage("DLDEBUG ["+getHost()+"] "+state.toString());
...
//        }
    ...
    public int getWeight() {
        ...
        if (hallmarkBalance == -1 || hallmarkBalanceHeight < Dwa.getBlockchain().getHeight() - 60) {
            long accountId = hallmark.getAccountId();
            Account account = Account.getAccount(accountId);
            hallmarkBalance = account == null ? 0 : account.getBalanceNQT();
            hallmarkBalanceHeight = Dwa.getBlockchain().getHeight();
        }
        return (int)(adjustedWeight * (hallmarkBalance / Constants.ONE_DWA) / Constants.MAX_BALANCE_DWA);
    }
    ...
    public void blacklist(Exception cause) {
        if (cause instanceof DwaException.NotCurrentlyValidException || cause instanceof BlockchainProcessor.BlockOutOfOrderException
    ...
    public void blacklist(String cause) {
        blacklistingTime = Dwa.getEpochTime();
    ...
    public JSONObject send(final JSONStreamAware request, int maxResponseSize) {
        ...
        try {
            ...
            if (useWebSocket && !webSocket.isOpen()){
                Logger.logDebugMessage("..["+host+"] startClient");
                useWebSocket = webSocket.startClient(URI.create("ws://" + host + ":" + getPort() + "/dwa"),this,request,maxResponseSize);
            }
            //
            ...
            if (useWebSocket) {
                ...
                if (maxResponseSize > 0) {
                    ...
                    if (wsResponse.length() > maxResponseSize)
                        throw new DwaException.DwaIOException("Maximum size exceeded: " + wsResponse.length());
                ...
                //
                URL url = new URL("http://" + host + ":" + getPort() + "/dwa");
                if (communicationLoggingMask != 0)
                ...
                if (connection.getResponseCode() == HttpURLConnection.HTTP_OK) {
                    ...
                    boolean isWorthyReconnect = isWorthyReconnect(host,webSocket,"connection.getResponseCode() == HttpURLConnection.HTTP_OK",new Exception("responseCode: "+connection.getResponseCode()));
                    if(!isWorthyReconnect){
                        deactivate();
...
//                Logger.logDebugMessage("PeerImpl ["+host+"] Check for an error response : Error");
                boolean isWorthy = isWorthyReconnect(host,webSocket,"response != null && response.get(\"error\") != null",new Exception("Error: "+response.get("error")));
                if(response.get("error").equals(Errors.DOWNLOADING)){
                    isWorthy = true;
                    resetCounterDWAReconnection(host);
                }
                if(!isWorthy) deactivate();
                if (Errors.SEQUENCE_ERROR.equals(response.get("error")) && request != Peers.getMyPeerInfoRequest()) {
                    ...
                    //Counter for Error / Blacklist, increment counter
                }
            }
        } catch (DwaException.DwaIOException e) {
            //Counter for Error / Blacklist, increment counter
            Exception feedE = e;
            if (feedE == null) feedE = new Exception("Null Ecxception");
            boolean isWorthy = isWorthyReconnect(host, webSocket, "DwaException.DwaIOException e", feedE);
            if (!isWorthy) {
                blacklist(e);
                if (connection != null) {
                    connection.disconnect();
            ...
            if(e instanceof IOException || e instanceof SocketTimeoutException || e instanceof SocketException || Errors.END_OF_FILE.equals(e.getMessage())){
                Logger.logDebugMessage("..["+host+"] "+e.getMessage()+", force reconnect, reason: "+e.toString());
                if (connection != null) {
                    connection.disconnect();
                }
                getDWAReconnectionParameter(host).resetCounter();
                new Timer().schedule(
                        new TimerTask() {
                            @Override
                            public void run() {
                                webSocket.close();
                                webSocket.nullifySession();
                                connect();
                            }
                        },10000);
                return response;
            ...
            //Counter for Error / Blacklist, increment counter
            Exception feedE = e;
            if(feedE == null) feedE = new Exception("null Exception");
            boolean isWorthy = isWorthyReconnect(host,webSocket,"RuntimeException|ParseException e",feedE);
            if(!isWorthy){
                deactivate();
                if (connection != null) {
                    connection.disconnect();
    ...
    synchronized DWAReconnectionParameter getDWAReconnectionParameter(String host){
        DWAReconnectionParameter dwaReconnectionParameter = mapReconnection.get(host);
        if(dwaReconnectionParameter==null) {
            dwaReconnectionParameter = new DWAReconnectionParameter(host, Constants.reconnectionDelay,Constants.reconnectionDelayMax);
            mapReconnection.put(host,dwaReconnectionParameter);
        }
        return dwaReconnectionParameter;
    }

    synchronized void increaseCounterDWAReconnectionParameter(String host){
        mapReconnection.get(host).increaseCounter();
    }

    synchronized long timerDWAReconnectionParameter(String host){
        return mapReconnection.get(host).getTimer();
    }

    synchronized boolean scheduleToConnectEqualCount(String host){
        return mapReconnection.get(host).scheduleToConnect==mapReconnection.get(host).count;
    }

    synchronized void resetCounterDWAReconnection(String host){
        getDWAReconnectionParameter(host).resetCounter();
    }
    ...
    void connect() {
        lastConnectAttempt = Dwa.getEpochTime();
        try {
            Logger.logDebugMessage("..["+host+"] Peers.ignorePeerAnnouncedAddress: "+Peers.ignorePeerAnnouncedAddress+", announcedAddress: "+announcedAddress);
            if (!Peers.ignorePeerAnnouncedAddress && announcedAddress != null) {
                try {
                    ...
                    if (!inetAddress.equals(InetAddress.getByName(host))) {
                        Logger.logDebugMessage("..["+host+"] !inetAddress.equals(InetAddress.getByName(host))");
            ...
            if (response != null) {
                Logger.logDebugMessage("..["+host+"] response != null, error: "+response.get("error"));
                ...
                if (announcedAddress == null) {
                    Logger.logDebugMessage("..["+host+"] announcedAddress == null");
                    if (hallmark == null || hallmark.getPort() == Peers.getDefaultPeerPort()) {
                        ...
                        Logger.logDebugMessage("..["+host+"] CheckPoint 2 "+hallmark);
                        setState(State.NON_CONNECTED);
                        return;
                    }
                }
                Logger.logDebugMessage("..["+host+"] CheckPoint 1");

                if (!isOldVersion) {
                    Logger.logDebugMessage("..["+host+"] CheckPoint 3 CONNECTED");
                    setState(State.CONNECTED);
                    if (services != origServices) {
                        Peers.notifyListeners(this, Peers.Event.CHANGED_SERVICES);
                    }
                } else if (!isBlacklisted()) {
                    Logger.logDebugMessage("..["+host+"] CheckPoint 4 Blacklisted isOldVersion");
                    blacklist("Old version: " + version);
                }
            } else {
                Logger.logDebugMessage("..["+host+"] Failed to connect to peer");
                setState(State.NON_CONNECTED);
            }
        } catch (RuntimeException e) {
            String message = e==null?"":e.toString();
            Logger.logDebugMessage("..["+host+"] RuntimeException "+message);
            blacklist(e);
        }
    }
    ...
    boolean analyzeHallmark(final String hallmarkString) {
        ...
        try {
            ...
            for (PeerImpl peer : groupedPeers) {
                peer.adjustedWeight = Constants.MAX_BALANCE_DWA * peer.getHallmarkWeight(mostRecentDate) / totalWeight;
