...
public class PeerWebSocket {
    ...
    private static HashMap<String, DWAReconnectionParameter> reconnectionParameter = new HashMap<>();
    private URI activeServerUri;
    private PeerImpl peer;
    private JSONStreamAware request;
     ...
     */
    public boolean startClient(URI uri, PeerImpl peer, JSONStreamAware request, int maxResponseSize) throws IOException {
        this.peer = peer;
        ...
        try {
            if (session != null) {
                ...
                peerClient.setConnectTimeout(Peers.connectTimeout+100);
                Future<Session> conn = peerClient.connect(this, uri, req);
                conn.get(Peers.connectTimeout + 100, TimeUnit.MILLISECONDS);
                useWebSocket = true;
                String identifier = activeServerUri.getHost();
                DWAReconnectionParameter dwaReconnectionParameter = reconnectionParameter.get(identifier);
                peer.resetCounterDWAReconnection(identifier);
                if(dwaReconnectionParameter!=null) dwaReconnectionParameter.resetCounter();
                Logger.logDebugMessage("..["+uri.getHost()+"] PeerWebSocket CONNECTED");
            }
            ...
            reconnect("TimeoutException","TimeoutException while doing startClient");
    ...
    public void onError(Throwable throwable){
        ...
        final String identifier = activeServerUri.getHost();
        Logger.logDebugMessage(".. ["+identifier+"] OnWebSocketError, reason: "+reason);
        // Only Connection refused able to reconnect. Otherwise reconnect through on close
        if(reason.equalsIgnoreCase("Connect Timeout")||(reason.equalsIgnoreCase("Connection refused")&&reason!=null)) reconnect("OnWebSocketError",reason);
//        if(reason.equalsIgnoreCase("Connect Timeout")) reconnect("OnWebSocketTimeout",reason);
    }

    public void reconnect(String source,String reason){
//        if(true)return;
        final String identifier = activeServerUri.getHost();
        DWAReconnectionParameter dwaReconnectionParameter = reconnectionParameter.get(identifier);
        if(dwaReconnectionParameter==null){
            dwaReconnectionParameter = new DWAReconnectionParameter(identifier, Constants.reconnectionDelay,Constants.reconnectionDelayMax);
            reconnectionParameter.put(identifier,dwaReconnectionParameter);
        }
        //check if blacklist, if blacklist, no need to reconnect
        boolean isBlacklisted = false;
        if(peer!=null) isBlacklisted = peer.isBlacklisted();
        if(isBlacklisted){
            Logger.logDebugMessage("..["+identifier+"] blacklisted, source: "+source+", reason: "+reason+", no need to reconnect...");
            return;
        }
        dwaReconnectionParameter.increaseCounter();
        
        final int dwaCount = dwaReconnectionParameter.count;
        long dwaTimer = dwaReconnectionParameter.getTimer();
        Logger.logDebugMessage("..["+identifier+"] Not blacklisted, Schedule to connect in "+dwaTimer+" ms, source: "+source+", reason: "+reason+", count: "+dwaCount);
        //Reconnection still worthy
        new Timer().schedule(
                new TimerTask() {
                    @Override
                    public void run() {
                        DWAReconnectionParameter dwaReconnectionParameter = reconnectionParameter.get(identifier);
                        if(dwaReconnectionParameter.scheduleToConnect>0&&dwaReconnectionParameter.scheduleToConnect!=dwaReconnectionParameter.count){
                            Logger.logDebugMessage("..["+identifier+"] Canceled count: "+dwaReconnectionParameter.count+" scheduled to connect "+dwaReconnectionParameter.scheduleToConnect);
                            return;
                        }
                        // your code here
                        Logger.logDebugMessage("..["+identifier+"] Start trying to connect... count: "+dwaCount);
                        try{
                            if (!isOpen()) {
                                close();
                                connectTime = 0;
                                session = null; // Start from all over again
    //                            peer.send(request,maxResponseSize);
                                peer.connect();
                                dwaReconnectionParameter.scheduleToConnect = 0;
                            }
                        }catch(Exception e){
    ...
    public void onClose(int statusCode, String reason) {
        ...
        Logger.logDebugMessage("["+activeServerUri.getHost()+"] OnWebSocketClose, reason: "+reason);
        lock.lock();
        try {
            if (session != null) {
                if ((Peers.communicationLoggingMask & Peers.LOGGING_MASK_200_RESPONSES) != 0) {
        ...
        reconnect("OnWebSocketClose",reason);
    }
