...
import dwa.Dwa;
import dwa.model.DWABlacklistParameter;
...
public final class PeerServlet extends WebSocketServlet {
    ...
    HashMap<String, DWABlacklistParameter> blacklistParameter = new HashMap<>();
    int blacklistingThresholdCount = Dwa.getIntProperty("dwa.blacklistingThresholdCount",25);
    int blacklistingThresholdErrorDuration = Dwa.getIntProperty("dwa.blacklistingThresholdErrorDuration",300000);
    ...
    private static final BlockchainProcessor blockchainProcessor = Dwa.getBlockchainProcessor();
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try (CountingOutputWriter writer = new CountingOutputWriter(resp.getWriter())) {
            ...
            if (peer != null) {
                ...
                //  peer.blacklist(e); //Original Implementation
                checkWorthyBlacklist(peer,e);
            }
            throw e;
        }
    }

    void checkWorthyBlacklist(PeerImpl peer,Exception e){
        DWABlacklistParameter dwaBlacklistParameter = blacklistParameter.get(peer.getHost());
        if(dwaBlacklistParameter==null){
            dwaBlacklistParameter = new DWABlacklistParameter(peer.getHost());
            blacklistParameter.put(peer.getHost(),dwaBlacklistParameter);
        }
        long now = Calendar.getInstance().getTimeInMillis();
        if((dwaBlacklistParameter.lastBlocked+blacklistingThresholdErrorDuration)<now){
            dwaBlacklistParameter.resetCounter();
        }
        dwaBlacklistParameter.increaseCounter();
        if(dwaBlacklistParameter.count>blacklistingThresholdCount){
            // It is exceed limit, then we need to blacklist it
            peer.blacklist(e);
        }
    }
    ...
    private JSONStreamAware process(PeerImpl peer, Reader inputReader) {
        ...
        try (CountingInputReader cr = new CountingInputReader(inputReader, Peers.MAX_REQUEST_SIZE)) {
            ...
            peer.setLastInboundRequest(Dwa.getEpochTime());
            ...
            resetPeerBlacklist(peer);
//            Logger.logDebugMessage("DLDEBUG PeerServlet:process");
    ...
    void resetPeerBlacklist(PeerImpl peer){
        DWABlacklistParameter dwaBlacklistParameter = blacklistParameter.get(peer.getHost());
        if(dwaBlacklistParameter!=null)dwaBlacklistParameter.resetCounter();
    }
