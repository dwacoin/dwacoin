...
final class TransactionDb {
    ...
    static TransactionImpl loadTransaction(Connection con, ResultSet rs) throws DwaException.NotValidException {
        try {
    ...
    static HashMap<String,ArrayList<Long>> saveTransactions(Connection con, List<TransactionImpl> transactions) {
        try {
            short index = 0;
            HashMap<String,ArrayList<Long>> notifications = new HashMap<>();
            for (TransactionImpl transaction : transactions) {
                try (PreparedStatement pstmt = con.prepareStatement("INSERT INTO transaction (id, deadline, "
                    ...
                    long resStatement = pstmt.executeUpdate();
                    if(resStatement>0){
                        //Insert Success
                        HashMap<String,String> sender = Constants.accountNotificationRecipient.get(""+transaction.getSenderId());
                        HashMap<String,String> recipient = Constants.accountNotificationRecipient.get(""+transaction.getRecipientId());
    ...
    static void addToList(long transactionId,ArrayList<Long> arrayList){
