...
final class BlockchainImpl implements Blockchain {
    ...
    @Override
    public DbIterator<TransactionImpl> getAllTransactionsOrderByTimestamp() {
        Connection con = null;
        try {
            con = Db.db.getConnection();
            PreparedStatement pstmt = con.prepareStatement("SELECT * FROM transaction ORDER BY timestamp DESC LIMIT "+ API.maxRecords);
            return getTransactions(con, pstmt);
        } catch (SQLException e) {
            DbUtils.close(con);
            throw new RuntimeException(e.toString(), e);
    ...
    public DbIterator<TransactionImpl> getTransactions(long accountId, int numberOfConfirmations, byte type, byte subtype,
        ...
        try {
            ...
            int prunableExpiration = Math.max(0, Constants.INCLUDE_EXPIRED_PRUNABLE && includeExpiredPrunable ?
                                        Dwa.getEpochTime() - Constants.MAX_PRUNABLE_LIFETIME :
                                        Dwa.getEpochTime() - Constants.MIN_PRUNABLE_LIFETIME);
            if (withMessage) {
