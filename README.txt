...
jar/exe/dmg package, or run "java -jar dwa-client.jar" to start the
installer. See https://bitbucket.org/JeanLucPicard/dwa/issues/283 for more
details about the IzPack installer. After installation, use the shortcuts or
desktop icons to start the Dwa server.


Using the dwa-client.zip package:

Unpack the dwa-client.zip package and open a shell in the resulting dwa
...
the dwa_db blockchain database. The start.sh script can be run from any
directory, it starts the java process in the background and uses ~/.dwa for
configuration files, logs, and the dwa_db database. Unlike run.sh, start.sh
...
message "Dwa server 1.x.x started successfully" in the console log. If run in
desktop mode, a JavaFX window will open automatically. Otherwise, open a
browser, without stopping the java process, and go to http://localhost:4876 ,
where the Dwa UI should now be available.
...
to the Dwa installation directory, as the use of special characters may result
...
conf/dwa-default.properties file. All possible settings are listed, with
detailed explanation. If you decide to change any setting, do not edit
dwa-default.properties directly, but create a new conf/dwa.properties file
and only add to it the properties that need to be different from the default
values. You do not need to delete the defaults from dwa-default.properties, the
settings in dwa.properties override those in dwa-default.properties. This way,
when upgrading the software, you can safely overwrite dwa-default.properties
with the updated file from the new package, while your customizations remain
safe in the dwa.properties file.


How to contribute?

There are many ways to contribute to Dwa. Here are some examples:
...
The Dwa software is a client-server application. It consists of a java server
...
The java process communicates with peers on port 4874 tcp by default. If you are
...
The user interface is available on port 4876. This port also accepts http API
requests which other Dwa client applications could use.

The blockchain is stored on disk using the H2 embedded database, inside the
dwa_db directory. When upgrading, you should not delete the old dwa_db
...
dwa_db, except that it will take some extra time to download the blockchain
from scratch.

The default Dwa client does not store any wallet-type file on disk. Unlike
...
which you see in the console window, but also to a file dwa.log, which gets
overwritten at restart. In case of an error, the dwa.log file may contain
helpful information, so include its contents when submitting a bug report.

In addition to the default user interface at http://localhost:4876 , the
following urls are available:

http://localhost:4876/test - a list of all available http API requests, very
...
http://localhost:4876/test?requestType=<specificRequestType> - same as above,
but only shows the form for the request type specified.

http://localhost:4876/doc - a javadoc documentation for client developers who
...
dwa.jar file using the enclosed jar.sh script, and then dwa.jar should be
